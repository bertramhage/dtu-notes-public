...
1. The variable i starts at 1 and doubles each iteration ($i = 2 * i$).
    
2. The loop continues until i exceeds $n^2$.
    

We can represent the values of i after each iteration:
...
To find the number of iterations k, solve the inequality:

$2^k \leq n^2 \implies k \leq \log_2(n^2) \implies k \leq 2 \log_2(n)$

  ### **Running Time Complexity:**

  

The loop runs for a maximum of $2 \log_2(n)$ iterations, which is **O(log n)**.
...
The running time of the algorithm ALG2(n) is **O(log n)**.